...
{
    public class when_executing_a_query_with_using_a_generic_type_inheriting_a_non_generic_known_provider :
        given.a_query_coordinator_with_non_generic_known_provider
    {
        ...
        It should_forward_query_with_clause_to_provider = () =>
            query_provider_mock.Verify(q => q.Execute(actual_query, paging), Times.Once());

        It should_filter_result = () => GetMock<IReadModelFilters>().Verify(r => r.Filter(items), Times.Once());
    }
}