...
namespace Bifrost.Web.Specs.Services.for_RequestParamsFactory.given
{
    public class a_request_params_factory
    {
        ...
        Establish context = () =>
        {
            ...
            serializer_mock
                .Setup(s => s.FromJson<Dictionary<string, string>>("mystring", null))
                .Returns(inputStreamParams);

            var inputStream = System.Text.Encoding.GetEncoding("iso-8859-1").GetBytes("mystring");
            ...
            http_request_base_mock.Setup(r => r.InputStream).Returns(new MemoryStream(inputStream));
