...
namespace Bifrost.Specs.Commands.for_CommandCoordinator
{
    ...
    public class when_handling_command_and_an_exception_occurs_during_validation : given.a_command_coordinator
    {
        ...
        Establish context = () =>
        {
            ...
            GetMock<ICommandValidators>().Setup(cvs => cvs.Validate(command)).Throws(exception);
        };

        Because of = () => result = coordinator.Handle(command);

        It should_have_authorized_the_command = () => GetMock<ICommandSecurityManager>().VerifyAll();
