...
namespace Bifrost.Specs.Execution.for_TypeFinder
{
    ...
    public class when_finding_types_with_only_one_implementation : given.a_type_finder
    {
        ...
        Establish context = () => GetMock<IImplementorFinder>().Setup(c => c.GetImplementorsFor(typeof(ISingle))).Returns(new[] { typeof(Single) });

        Because of = () => type_found = type_finder.FindSingle<ISingle>(Get<IImplementorFinder>());
